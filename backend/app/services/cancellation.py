...
NOTE: This is a plain in-process dict. It only works correctly with a single
backend worker/process. If this service is ever scaled to multiple processes
or instances, this must be replaced with a shared store (e.g. Redis) so every
worker sees the same cancel flags. Confirmed single-process deployment for now.
"""
...
def cancel(request_id: str) -> bool:
    """Returns True if the request_id was known and got flagged, False if unknown."""
    with _lock:
...
def _sweep_stale_locked() -> None:
    now = time.time()
    stale = [rid for rid, ts in _registered_at.items() if now - ts > _MAX_AGE_SECONDS]
    for rid in stale:
        _flags.pop(rid, None)
