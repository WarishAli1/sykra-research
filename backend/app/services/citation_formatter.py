...
def _bibtex_key(paper: dict) -> str:
    first_author = (
        paper.get("authors", ["unknown"])[0].split()[-1].lower()
        if paper.get("authors")
        else "unknown"
    )
    year = str(paper.get("published", "n.d."))[:4]
    title_word = re.sub(r"[^a-zA-Z]", "", paper.get("title", "paper").split()[0]).lower()
...
def format_apa(paper: dict) -> str:
    ...
    return f"{authors} ({year}). {paper.get('title', 'Untitled')}. {label}. {paper.get('link', '')}"
...
def format_ieee(paper: dict, index: int = 1) -> str:
    ...
    return (
        f'[{index}] {authors}, "{paper.get("title", "Untitled")}," {year}. '
        f'[Online]. Available: {paper.get("link", "")}'
    )
