...
Does it correctly and sufficiently address its part of the question?
Is it factually grounded in the cited sources?
Does it contain obvious hallucinations or unsupported claims?
...
def critique_node(state: AgentState) -> AgentState:
    ...
    # Reduced truncation limits to guarantee we stay well under token limits
    sections_block = "\n\n".join(
        f"### {s.get('title', s.get('module_id', ''))}\n"
        f"{s.get('content', '')[:1000]}"
            ...
            [
                ...
                HumanMessage(
                    content=_SECTION_CRITIQUE_PROMPT.format(
                        query=query,
                        sections_block=sections_block[:4000],
                    )
                ),
            ],
            max_tokens=1024,  # CRITICAL FIX: Prevents Groq 413 Request Too Large error
