import hashlib
import time
from concurrent.futures import ThreadPoolExecutor, as_completed, TimeoutError
from app.agents.state import AgentState
from app.services.paper_search import search_arxiv, search_openalex
from app.services.embeddings import embed_texts, similarity
from app.services.embedding_cache import batch_get_or_compute

_query_cache: dict[str, tuple[float, list[dict]]] = {}
CACHE_TTL_SECONDS = 3600
MAX_TOTAL_CANDIDATES = 150

def _get_cached(query: str) -> list[dict] | None:
    key = hashlib.sha256(query.lower().encode()).hexdigest()
    entry = _query_cache.get(key)
    if entry and (time.time() - entry[0]) < CACHE_TTL_SECONDS:
        return entry[1]
    return None

def _set_cached(query: str, results: list[dict]):
    key = hashlib.sha256(query.lower().encode()).hexdigest()
    _query_cache[key] = (time.time(), results)

def search_node(state: AgentState) -> AgentState:
    if state.get("evidence_mode") == "uploaded":
        return {**state, "raw_search_results": [], "needs_retry": False}

    terms = state.get("search_queries") or state.get("search_terms") or [state["query"]]
    terms = list(dict.fromkeys(terms))  

    to_fetch = [t for t in terms if _get_cached(t) is None]
    per_term_results = {t: _get_cached(t) or [] for t in terms}

    if to_fetch:
        with ThreadPoolExecutor(max_workers=8) as ex:
            futures = {}
            for term in to_fetch:
                futures[ex.submit(search_arxiv, term)] = ("arxiv", term)
                futures[ex.submit(search_openalex, term, 10, True)] = ("openalex", term)

            gathered = {term: [] for term in to_fetch}
            for future in as_completed(futures):
                source, term = futures[future]
                try:
                    res = future.result(timeout=15)
                except (TimeoutError, Exception):
                    res = []
                gathered[term].extend(res)

            for term, results in gathered.items():
                _set_cached(term, results)
                per_term_results[term] = results

    seen_ids, seen_titles = set(), set()
    deduped = {}
    for term, results in per_term_results.items():
        for p in results:
            arxiv_id = p.get("arxiv_id")
            openalex_id = p.get("openalex_id")
            norm = p.get("title", "").strip().lower()
            if not norm:
                continue
            if arxiv_id and arxiv_id in seen_ids:
                continue
            if openalex_id and openalex_id in seen_ids:
                continue
            if norm in seen_titles:
                continue
            if arxiv_id:
                seen_ids.add(arxiv_id)
            if openalex_id:
                seen_ids.add(openalex_id)
            seen_titles.add(norm)
            p["_source_term"] = term
            deduped[norm] = p
    all_papers = list(deduped.values())

    query_words = [w for w in state["query"].lower().split() if len(w) > 3]
    if query_words:
        filtered = [p for p in all_papers if any(qw in p["title"].lower() for qw in query_words)]
        if len(filtered) >= 5:
            all_papers = filtered

    mandatory_kws = state.get("mandatory_domain_keywords")
    if mandatory_kws:
        hard = [p for p in all_papers if any(
            kw in (p.get("title", "") + " " + p.get("summary", "")).lower()
            for kw in mandatory_kws
        )]
        if len(hard) < 5:
            domain_full = state.get("domain_full", "")
            soft_words = domain_full.lower().split() if domain_full else []
            soft = [p for p in all_papers if p not in hard and any(
                w in (p["title"] + p.get("summary", "")).lower() for w in soft_words
            )]
            all_papers = hard + soft[:max(0, 5 - len(hard))]
        else:
            all_papers = hard

    CANDIDATE_PRE_EMBED = 100
    candidates = all_papers[:CANDIDATE_PRE_EMBED]

    query_embedding = embed_texts([state["query"]])[0]

    paired = batch_get_or_compute(candidates, embed_texts) 

    SIM_THRESH = 0.35
    filtered_papers = []
    for paper, vec in paired:
        sim = similarity(query_embedding, vec)
        if sim >= SIM_THRESH:
            paper["_initial_sim"] = sim
            filtered_papers.append(paper)

    if len(filtered_papers) < 5:
        filtered_papers = [p for p, _ in paired]
        for p, vec in paired:
            if not p.get("_initial_sim"):
                p["_initial_sim"] = similarity(query_embedding, vec)

    combined = filtered_papers[:MAX_TOTAL_CANDIDATES]

    search_attempts = state.get("search_attempts", 0) + 1
    needs_retry = len(combined) < 5 and search_attempts < state.get("max_search_attempts", 2)

    return {
        **state,
        "raw_search_results": combined,
        "search_attempts": search_attempts,
        "needs_retry": needs_retry,
        "query_embedding": query_embedding,
    }